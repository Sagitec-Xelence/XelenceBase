...
cy.login()
cy.get('#actOpenMessages').click()
cy.get('#actNew').click()
cy.contains('[ Record displayed. Please make changes and press SAVE. ]').should('be.visible')
cy.get('#txtMessageID').type(uniqueSeed)
var random_string = hp.GetRandomString()
cy.get('#txtDisplayMessage').should('be.visible').type('test'+random_string)
cy.get('#ddlSeverity').select('Error').should('have.value','E')
var random_string = hp.GetRandomString()
cy.get('#txtEmployerInstructions').should('be.visible').type('test'+random_string)
var random_string = hp.GetRandomString()
cy.get('#txtInternalInstructions').should('be.visible').type('test'+random_string)
...
it('Lookup -->reset', () => {
  ...
  cy.get('#actOpen').click({force:true})
  cy.wait(5000)
 ...
 // cy.get('[id^=wfmMessagesLookup]').find('#div#GlobalMessageDiv').should('have.text'," No record selected. Please select record(s) and try again.")
  cy.get('#actDelete').click({force:true})
  cy.wait(5000)
 // cy.contains(' No record(s) present in grid to export.').should('be.visible')
 // cy.wait(5000)
 ...
 cy.wait(3000)
  ...
  it('Lookup -->Store State', () => {
    ...
    cy.login()
    cy.get('#actOpenMessages').click()
    cy.get('#txtMessageId').type(162400629)
